...
def load_mnist():
    X_train = gzip.open("mnist/train-images-idx3-ubyte.gz").read()
    X_train = np.frombuffer(X_train, dtype=np.uint8)[0x10:].reshape((-1, 28*28)).astype(np.float32) 

    Y_train = gzip.open("mnist/train-labels-idx1-ubyte.gz").read()
    Y_train = np.frombuffer(Y_train, dtype=np.uint8)[8:]
    
    X_test = gzip.open("mnist/t10k-images-idx3-ubyte.gz").read()
    X_test = np.frombuffer(X_train, dtype=np.uint8).reshape((-1, 28*28)).astype(np.float32) 

    Y_test = gzip.open("mnist/t10k-labels-idx1-ubyte.gz").read()
